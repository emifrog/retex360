import * as Sentry from "@sentry/nextjs";

Sentry.init({
  dsn: "https://05671b4136141cae9628dbfe9227d11f@o4510308585046016.ingest.de.sentry.io/4510427321139280",

  // Performance Monitoring
  tracesSampleRate: 1.0, // Réduire à 0.1-0.2 en production

  // Environnement
  environment: process.env.NODE_ENV,

  // Capturer les erreurs non gérées
  integrations: [
    Sentry.prismaIntegration(), // Si vous utilisez Prisma plus tard
  ],

  // Debug en développement uniquement
  debug: process.env.NODE_ENV === "development",

  // Filtrer les erreurs non pertinentes côté serveur
  beforeSend(event, hint) {
    const error = hint.originalException as Error;

    // Ignorer les erreurs d'authentification attendues
    if (error?.message?.includes("NEXT_REDIRECT")) {
      return null;
    }
    if (error?.message?.includes("NEXT_NOT_FOUND")) {
      return null;
    }

    return event;
  },
});
