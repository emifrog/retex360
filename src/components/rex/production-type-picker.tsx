  ...
  return (
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        {(
          Object.entries(PRODUCTION_TYPE_RULES) as [
            ProductionType,
            (typeof PRODUCTION_TYPE_RULES)[ProductionType],
          ][]
        ).map(([type, config]) => {
          ...
          return (
            <button
              ...
              className={cn(
                ...
                isSelected
                  ? cn('border-primary bg-primary/5 dark:bg-primary/10', 'ring-2 ring-primary/20')
                  : cn(
            ...
            >
              ...
              {/* Icon */}
              <div
                className={cn('p-2.5 rounded-lg mb-3', isSelected ? config.bgColor : 'bg-muted')}
              >
                <Icon
                  className={cn('w-5 h-5', isSelected ? config.textColor : 'text-muted-foreground')}
                />
              </div>
      ...
      <p className="text-xs text-muted-foreground">
        {value === 'signalement' && (
          <>
            <strong>Signalement :</strong> Remontée rapide avec les informations essentielles. Idéal
            pour une première alerte.
          </>
        )}
        {value === 'pex' && (
          <>
